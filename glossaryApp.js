...
const APP_VERSION='3.0.126',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
let bookmarkLearningReverse=false;
...
function renderTodayBookmarks(){screen='today-bookmarks';app.innerHTML=`<section class="view-head"><span></span><h1>今日の10語ブックマーク</h1><span></span></section><div id="list-container" class="today-bookmark-list">${todayBookmarkCards()}</div>`;syncFloatingNav();scrollTo(0,0)}
function startTodayBookmarkSession(preferredId=0){const terms=todayBookmarkTerms();if(!terms.length){renderTodayBookmarks();return}screen='session';flashcardMode=false;session=[...terms];sessionIndex=Math.max(0,session.findIndex(term=>term.id===Number(preferredId)));sessionId=`tb-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;sessionModeKey='todayBookmarks';statusSessionMode=false;isTodaySession=false;todayQuizMode=true;todayAnswers=new Map();evaluatedIds=new Set();sessionStats={label:'ブックマーク問題',sourceStatus:'',studied:0,correct:0,firstCorrect:0,secondCorrect:0,wrong:0,unable:0,safe:0,caution:0,danger:0};prepareTerm();renderSession();scrollTo(0,0)}
...
function nextTerm(){if(todayQuizMode){const response=todayResponse();countRoundProgress(response);if(response.completed&&!response.counted){sessionStats.studied++;response.counted=true;saveRecentAnswer(currentTerm(),response)}}else sessionStats.studied++;if(sessionIndex<session.length-1){sessionIndex++;persistTodaySession();prepareTerm();renderSession();scrollTo(0,0)}else if(sessionModeKey==='todayBookmarks'){renderTodayBookmarks()}else{saveCategoryRound();renderResult()}}
function previousTerm(){if(sessionIndex>0){sessionIndex--;persistTodaySession();prepareTerm();renderSession();scrollTo(0,0)}else{if(isTodaySession){todayMeta().activeSession=null;saveTodayMeta()}if(sessionModeKey==='todayBookmarks')renderTodayBookmarks();else renderHome()}}
...
function exitCurrent(){if(isTodaySession){todayMeta().activeSession=null;saveTodayMeta()}if(sessionModeKey==='todayBookmarks')renderTodayBookmarks();else if(sessionModeKey==='learningBookmarks')renderBookmark();else renderHome()}
...
function learningPool(category=flashCategory){if(sessionModeKey==='learningBookmarks')return bookmarkedTerms();return flashcardTerms.filter(term=>!category||term.category===category)}
