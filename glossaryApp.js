...
function categoryStatsHtml(){const rows=categoryStats(),total=rows.reduce((sum,row)=>sum+row.total,0);return `<section class="category-stats"><h2 class="category-stats-title">カテゴリ別成績：計${total}問</h2>${rows.map(row=>{const currentRound=row.round.completed+1,history=[...row.round.history].reverse();return `<div class="category-row"><div class="category-row-main"><button type="button" class="category-row-study" onclick="Glossary.startCategoryWeak('${esc(row.category)}')"><span class="category-row-heading"><span class="category-row-name">${esc(row.category)}：${row.total}問</span><span class="category-row-round">${lapLabel(currentRound)}</span></span>${categoryBarHtml(row)}</button></div>${history.length?`<details class="category-history"><summary>過去の周回成績</summary>${history.map(item=>`<div class="category-history-row"><span>${lapLabel(item.round)}</span>${categoryBarHtml(item)}</div>`).join('')}</details>`:''}</div>`}).join('')}</section>`}
