...
const hasLeadingBullet=value=>/^・/u.test(String(value||''));
const withoutLeadingBullet=value=>String(value||'').replace(/^(?:・\s*)+/u,'').trimStart();
...
const cleanPoint=value=>withoutLeadingBullet(value).trim();
...
const APP_VERSION='3.0.158',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function mixupComparisonHtml(term,linkable=true){
 ...
 return `<div class="mixup-comparison" data-multiline>${group.terms.map(item=>`<section class="mixup-comparison-term"><div class="mixup-term-name">${render(item.name)}</div><ul>${(item.points||[]).map(point=>`<li><span aria-hidden="true">・</span><span>${render(withoutLeadingBullet(point))}</span></li>`).join('')}</ul></section>`).join('')}<section class="mixup-memory"><div>${sectionHeading('memory')}</div>${(group.memory||[]).map(value=>`<p><span aria-hidden="true">・</span><span>${esc(withoutLeadingBullet(value))}</span></p>`).join('')}</section></div>`;
}
...
const EXAM_FIELDS=Object.freeze([
 ...
 {name:'文化論',description:'歴史や文化的背景の知識',categories:[]},
 {name:'理容技術理論',description:'カットやパーマなど技術の理論',categories:['カッティング']},
 {name:'運営管理',description:'経営・運営していく上で必要な知識',categories:[]}
]);
...
function startFieldQuiz(index){const field=EXAM_FIELDS[Number(index)];if(field)startGroupedQuiz(field.categories,field.name)}
...
function startFieldLearning(index,isReverse=false){const field=EXAM_FIELDS[Number(index)];if(field)startGroupedLearning(field.categories,field.name,isReverse)}
...
function categoryRowHtml(row){const currentRound=row.round.completed+1,history=[...row.round.history].reverse();return `<div class="category-row"><div class="category-row-main"><div class="category-row-study"><span class="category-row-heading"><span class="category-row-name">${esc(row.category)}：${row.total}語</span><span class="category-row-round">${lapLabel(currentRound)}</span></span>${categoryBarHtml(row)}</div><button type="button" class="category-reset-button" onclick="Glossary.resetCategoryLearning('${esc(row.category)}',event)" aria-label="${esc(row.category)}の成績をリセット">${resetIconHtml()}</button></div><div class="category-learning-actions" aria-label="${esc(row.category)}の学習方法"><button type="button" onclick="Glossary.startCategoryQuiz('${esc(row.category)}')">［3択］</button><button type="button" onclick="Glossary.startCategoryLearning('${esc(row.category)}',false)">［用語］</button><button type="button" onclick="Glossary.startCategoryLearning('${esc(row.category)}',true)">［逆引］</button></div>${history.length?`<details class="category-history"><summary>過去の周回成績</summary>${history.map(item=>`<div class="category-history-row"><span>${lapLabel(item.round)}</span>${categoryBarHtml(item)}</div>`).join('')}</details>`:''}</div>`}
function categoryStatsHtml(){const rows=categoryStats(),total=rows.reduce((sum,row)=>sum+row.total,0),rowMap=new Map(rows.map(row=>[row.category,row]));return `<section class="category-stats"><h2 class="category-stats-title">科目別成績：計${total}語</h2>${EXAM_FIELDS.map((field,index)=>{const fieldRows=field.categories.map(category=>rowMap.get(category)).filter(Boolean),terms=fieldRows.flatMap(row=>row.terms),breakdown=categoryBreakdown(terms),hasTerms=terms.length>0;return `<details class="field-row${hasTerms?'':' field-row-empty'}"${hasTerms?'':' aria-disabled="true"'}><summary><span class="field-row-heading"><span class="field-row-title"><span class="field-row-name">${esc(field.name)}</span><span class="field-row-description">${esc(field.description)}</span></span>${hasTerms?categoryBarHtml(breakdown):''}</span><span class="field-row-meta">${terms.length}語${hasTerms?'<span class="field-row-arrow" aria-hidden="true">›</span>':''}</span></summary>${hasTerms?`<div class="field-learning-actions" aria-label="${esc(field.name)}の学習方法"><button type="button" onclick="Glossary.startFieldQuiz(${index})">［3択］</button><button type="button" onclick="Glossary.startFieldLearning(${index},false)">［用語］</button><button type="button" onclick="Glossary.startFieldLearning(${index},true)">［逆引］</button></div><div class="field-row-content">${fieldRows.map(categoryRowHtml).join('')}</div>`:''}</details>`}).join('')}</section>`}
...
function termSymptoms(term){const disease=infectionDiseaseFor(term);if(disease)return Array.isArray(disease.symptoms)?disease.symptoms:[];return Array.isArray(term.symptoms)?term.symptoms:[]}
function symptomListHtml(values,currentId=0,linkable=false){return `<div data-multiline>${values.map(value=>{const text=withoutLeadingBullet(value),content=linkable?dictionaryText(text,currentId):esc(text);return `<div class="exam-bullet-line"><span aria-hidden="true">・</span><span class="exam-line-text">${content}</span></div>`}).join('')}</div>`}
...
function infectionDiseaseFor(term){if(term.category!=='感染症法')return null;const diseases=globalThis.INFECTION_DISEASE_DATA?.diseases||[];if(Number.isInteger(term.infectionDiseaseId)){const byId=diseases.find(item=>item.id===term.infectionDiseaseId);if(byId)return byId}return diseases.find(item=>item.name===term.name||item.name.startsWith(`${term.name}（`))||diseases.find(item=>item.alias&&(item.alias===term.name||term.aliases?.includes(item.alias)))||null}
...
function sessionNavigation(){const bookmarkMode=sessionModeKey==='todayBookmarks',lastLabel=bookmarkMode?'一覧へ戻る＞':'結果を見る＞';return `<nav class="session-navigation today-session-navigation" aria-label="${bookmarkMode?'ブックマーク問題':'今日の10語'}の前後移動"><button onclick="Glossary.previousTerm()">＜前へ</button><button class="session-next" onclick="Glossary.nextTerm()">${sessionIndex===session.length-1?lastLabel:'次へ＞'}</button></nav>`}
...
function nextTerm(){if(todayQuizMode){const response=todayResponse();countRoundProgress(response);if(response.completed&&!response.counted){sessionStats.studied++;response.counted=true;saveRecentAnswer(currentTerm(),response)}}else sessionStats.studied++;if(sessionIndex<session.length-1){sessionIndex++;persistTodaySession();prepareTerm();renderSession();scrollTo(0,0)}else if(sessionModeKey==='todayBookmarks'){renderTodayBookmarks(true)}else renderResult()}
function previousTerm(){if(sessionIndex>0){sessionIndex--;persistTodaySession();prepareTerm();renderSession();scrollTo(0,0)}else{if(isTodaySession){todayMeta().activeSession=null;saveTodayMeta()}if(sessionModeKey==='todayBookmarks')renderTodayBookmarks(true);else renderHome()}}
...
function restoreHomePosition(scrollY){renderHome();requestAnimationFrame(()=>scrollTo(0,Number(scrollY)||0))}
function exitCurrent(){if(screen==='today-bookmarks'){renderHome();return}if(isTodaySession){todayMeta().activeSession=null;saveTodayMeta()}if(flashcardMode&&sessionContext){returnFromLearningContext();return}if(sessionModeKey==='todayBookmarks')renderTodayBookmarks(true);else if(sessionModeKey==='categoryAll')restoreHomePosition(categoryHomeScrollY);else renderHome()}
...
function renderLearningCard(){
 ...
 const reverseHintsHtml=`<span class="reverse-hints-list">${reverseHints.map(value=>`<span class="reverse-hint-item"><span class="reverse-hint-text">${esc(hasLeadingBullet(value)?withoutLeadingBullet(value):value)}</span></span>`).join('')}</span>`;
 ...
 const examText=term.exam?.length?term.exam.map(value=>{const bullet=hasLeadingBullet(value),text=bullet?withoutLeadingBullet(value):value;return `<div class="${bullet?'exam-bullet-line':'exam-plain-line'}">${bullet?'<span aria-hidden="true">・</span>':''}<span class="exam-line-text">${linkText(text)}</span></div>`}).join(''):'';
 const symptoms=termSymptoms(term);
...
const learningCategories=['感染症予防','消毒法','公衆衛生','地域保健法','健康増進法','消費者基本法','皮膚','毛髪','香粧品成分','カッティング'];
...
function learningPool(category=flashCategory){if(sessionModeKey==='learningBookmarks')return bookmarkedTerms();if(sessionModeKey==='statusLearning')return listTerms;if(sessionModeKey==='searchLearning')return activeSearchTerms;return flashcardTerms.filter(term=>!category||term.category===category)}
function learningDeckKey(category=flashCategory){return category||'すべて'}
...
function startLearningMode(isReverse,preferredId=0){
 ...
 loadLearningDeck(preferred?preferred.category:flashCategory,preferred);
...
selectFlashCategory=function(category){if(category!==''&&!learningCategories.includes(category))return;saveLearningDeckPosition();loadLearningDeck(category);reverseRevealed=false;prepareTerm();renderSession();scrollTo(0,0)}
