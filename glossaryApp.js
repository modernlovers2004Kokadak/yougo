...
const APP_VERSION='3.0.175',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function sessionNavigation(){const bookmarkMode=sessionModeKey==='todayBookmarks',lastLabel=bookmarkMode?'一覧へ戻る＞':'結果を見る＞',response=todayQuizMode?todayResponse():null;return `<nav class="session-navigation today-session-navigation" aria-label="${bookmarkMode?'ブックマーク問題':'今日の10語'}の前後移動"><button onclick="Glossary.previousTerm()">＜前へ</button><button class="session-next" onclick="Glossary.nextTerm()">${sessionIndex===session.length-1?lastLabel:'次へ＞'}</button>${response?'<button type="button" class="point-trigger" onclick="Glossary.openPoint(event)">Point！</button>':''}</nav>`}
