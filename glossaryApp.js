...
function mixupComparisonHtml(term,linkable=true){
 ...
 return `<div class="mixup-comparison" data-multiline>${group.terms.map(item=>`<section class="mixup-comparison-term"><div class="mixup-term-name">${render(item.name)}</div><ul>${(item.points||[]).map(point=>`<li><span aria-hidden="true">・</span><span>${render(String(point).replace(/^・/u,''))}</span></li>`).join('')}</ul></section>`).join('')}<section class="mixup-memory"><div>【覚え方】</div>${(group.memory||[]).map(value=>`<p><span aria-hidden="true">・</span><span>${esc(String(value).replace(/^・/u,''))}</span></p>`).join('')}</section></div>`;
}
function mixupFieldHtml(term,heading='div',linkable=true){
 const body=mixupComparisonHtml(term,linkable);
 if(!body)return '';
 return `<div class="term-field mixup-field"><${heading} class="term-label mixup">${heading==='h2'?'混同注意':esc('混同注意')}</${heading}><div class="term-value" data-multiline>${body}</div></div>`;
}
...
function renderBookmark(){screen='bookmark';listTerms=data.terms.filter(term=>isBookmarked(term));app.innerHTML=`<section class="view-head"><span></span><h1>ブックマーク</h1><span></span></section><div id="list-container">${listCards(listTerms)}</div>`;syncFloatingNav()}
...
function listCards(terms,showNumber=false){return terms.length?terms.map(term=>{const s=termState(term),removeButton=screen==='bookmark'?`<button type="button" class="bookmark-list-remove" onclick="Glossary.removeBookmark(${term.id},event)">🔖解除</button>`:'',fields=screen==='bookmark'?termFields(term,true):termFields(term);return `<details class="term-card" id="term-${term.id}"><summary><span class="arrow">▶</span><span>${esc(term.name)}</span>${listMetaHtml(term,showNumber)}</summary><div class="term-back">${fields}${sourceBlock(term)}<div class="assessment-inline">${assessmentButtons(term,s?.status,'inline')}</div>${removeButton}</div></details>`}).join(''):'<div class="empty">対象の用語はありません。</div>'}
function todayBookmarkCards(){const terms=[...todayBookmarks].map(id=>termById.get(id)).filter(term=>term&&q[term.id]);return terms.length?terms.map(term=>{const question=q[term.id],options=[question.correct,...(question.distractors||[]).slice(0,2)];return `<details class="term-card today-bookmark-card" id="today-bookmark-${term.id}"><summary><span class="arrow">▶</span><span>${esc(term.name)}</span><span class="term-category">問題 ${term.id}<br>${esc(term.category)}</span></summary><div class="term-back"><div class="term-field"><div class="term-label">問題文</div><div class="term-value">${dictionaryText(question.question,term.id)}</div></div><div class="term-field"><div class="term-label">選択肢</div><div class="term-value"><ol>${options.map(value=>`<li>${esc(value)}</li>`).join('')}</ol></div></div><div class="term-field"><div class="term-label">正答</div><div class="term-value">${esc(question.correct)}</div></div><div class="term-field"><div class="term-label">解説</div><div class="term-value">${dictionaryText(question.explanation||term.definition||'',term.id)}</div></div><button type="button" class="today-bookmark-remove" onclick="Glossary.toggleTodayBookmark(${term.id},event)">🔖 解除</button></div></details>`}).join(''):'<div class="empty">登録した問題はありません。</div>'}
function renderTodayBookmarks(){screen='today-bookmarks';app.innerHTML=`<section class="view-head"><span></span><h1>今日の10語ブックマーク</h1><span></span></section><div id="list-container">${todayBookmarkCards()}</div>`;syncFloatingNav();scrollTo(0,0)}
function textField(label,value,extra='',linkable=false,currentId=0){if(!value)return '';const multi=String(value).split('<br>').length>1?' data-multiline':'';return `<div class="term-field ${extra}"><div class="term-label">${esc(label)}</div><div class="term-value${multi}">${linkable?dictionaryText(value,currentId):esc(value)}</div></div>`}
function listField(label,values,extra='',linkable=false,currentId=0){if(!values?.length)return '';const multi=values.length>1?' data-multiline':'';const isExam=label==='試験の要点';return `<div class="term-field ${extra}"><div class="term-label">${esc(label)}</div><div class="term-value${multi}"><ul${isExam?' class="exam-list"':''}>${values.map(value=>{const bullet=isExam&&value.startsWith('・'),text=bullet?value.slice(1).trimStart():value,content=linkable?dictionaryText(text,currentId):esc(text);return `<li${isExam?` class="${bullet?'exam-bullet':'exam-plain'}"`:''}>${bullet?'<span aria-hidden="true">・</span>':''}${isExam?`<span class="exam-line-text">${content}</span>`:content}</li>`}).join('')}</ul></div></div>`}
function termFields(term,linkable=false){return `${textField('定義',term.definition,'',linkable,term.id)}${term.aliases?.length?textField('別名・関連表記',term.aliases.join('／')):''}${Object.entries(term.special||{}).map(([label,value])=>textField(label,value)).join('')}${listField('試験の要点',term.exam,'exam-points',linkable,term.id)}${term.mixup?.length?mixupFieldHtml(term,'div',linkable):''}`}
...
function quizTermFields(term,linkable=false){const disease=infectionDiseaseFor(term);if(!disease)return termFields(term,linkable);return `${textField('正式名称',disease.name,'',linkable,term.id)}${textField('別名・通称',disease.alias||'なし','',linkable,term.id)}${textField('感染症法上の分類',disease.classification,'',linkable,term.id)}${textField('主な感染経路',disease.route,'',linkable,term.id)}${textField('代表的・特徴的な症状',disease.symptoms,'',linkable,term.id)}`}
function flashcardFields(term){return `${textField('定義',term.definition,'',true)}${listField('試験の要点',term.exam,'exam-points')}${term.mixup?.length?mixupFieldHtml(term,'div',true):''}`}
...
function searchTermNames(value){const host=document.getElementById('termSearchResults');if(!host)return;const raw=String(value||'').trim(),query=raw.toLocaleLowerCase('ja');if(!query){host.innerHTML='';return}const numberMatch=raw.match(/^(?:問題\s*)?(\d+)$/u),questionId=numberMatch?Number(numberMatch[1]):null;const matches=data.terms.filter(term=>(questionId!==null&&Number(term.id)===questionId)||term.name.toLocaleLowerCase('ja').includes(query)||(term.reading||'').includes(query)).slice(0,30);host.innerHTML=matches.length?matches.map(term=>`<button type="button" onclick="Glossary.openSearchedTerm(${term.id})"><span>${esc(term.name)}</span><small>問題 ${term.id}　${esc(term.category)}</small></button>`).join(''):'<p>該当する用語はありません。</p>'}
...
function renderLearningCard(){
 ...
 const details=`<section class="learning-details"><section><h2 class="definition">定義</h2><div${multilineAttr(defText)}>${defText}</div></section>${term.exam?.length?`<section><h2 class="point">試験の要点</h2><div${multilineAttr(examText)}>${examText}</div></section>`:''}${term.mixup?.length?mixupFieldHtml(term,'h2',!reverse||reverseRevealed):''}</section>`;
...
function learningPool(category=flashCategory){return flashcardTerms.filter(term=>!category||term.category===category)}
