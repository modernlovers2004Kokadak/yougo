...
const APP_VERSION='3.0.163',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function roundMeta(){const raw=learning[ROUND_META_KEY];if(raw&&typeof raw==='object'&&!Array.isArray(raw)){raw.history=(Array.isArray(raw.history)?raw.history:[]).filter(item=>Number(item.unlearned)===0&&Number(item.total)===data.terms.length);raw.completed=raw.history.reduce((maximum,item)=>Math.max(maximum,Number(item.round)||0),0);raw.current=raw.current&&typeof raw.current==='object'&&!Array.isArray(raw.current)?raw.current:{answers:{}};raw.current.answers=raw.current.answers&&typeof raw.current.answers==='object'&&!Array.isArray(raw.current.answers)?raw.current.answers:{};if(raw.schema!==2){raw.current={answers:{}};raw.schema=2;saveLearning()}return raw}return learning[ROUND_META_KEY]={schema:2,completed:0,history:[],current:{answers:{}}}}
