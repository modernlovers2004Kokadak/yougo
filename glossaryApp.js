...
const APP_VERSION='3.0.129',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
const SECTION_HEADINGS=Object.freeze({definition:'★定義★',exam:'★試験の要点★',mixup:'★混同注意★',memory:'★覚え方★'});function sectionHeading(key){return SECTION_HEADINGS[key]||''}
...
function renderStatusList(status,terms,label=states[status],restoreScroll=false){screen='status-list';flashcardMode=false;activeStatusListKey=status;activeStatusListLabel=label;listTerms=[...terms];app.innerHTML=`<section class="view-head"><span></span><h1>${esc(label)}の用語一覧</h1><span></span></section><div class="status-list-count">${terms.length}語</div><div id="list-container" class="status-learning-list">${statusLearningListHtml(terms)}</div>`;syncFloatingNav();requestAnimationFrame(()=>scrollTo(0,restoreScroll?statusListScrollY:0))}
function startStatusLearningSession(preferredId=0){if(!listTerms.length){renderStatusList(activeStatusListKey,[],activeStatusListLabel);return}statusListScrollY=window.scrollY;screen='session';flashcardMode=true;reverseMode=false;reverseRevealed=false;sessionModeKey='statusLearning';flashCategory='';session=[...listTerms];sessionIndex=Math.max(0,session.findIndex(term=>term.id===Number(preferredId)));sessionHistory=[sessionIndex];sessionHistoryIndex=0;sessionId=`sl-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;statusSessionMode=true;isTodaySession=false;todayQuizMode=false;todayAnswers=new Map();evaluatedIds=new Set();sessionStats={label:`${activeStatusListLabel}の用語`,sourceStatus:activeStatusListKey,studied:0,correct:0,firstCorrect:0,secondCorrect:0,wrong:0,unable:0,safe:0,caution:0,danger:0};prepareTerm();renderSession();scrollTo(0,0)}
...
function renderBookmark(restoreScroll=false){screen='bookmark';flashcardMode=false;listTerms=bookmarkedTerms();app.innerHTML=`<section class="view-head"><span></span><h1>ブックマーク</h1><span></span></section><div class="learning-bookmark-mode" role="group" aria-label="ブックマークの学習方法"><button type="button" class="${bookmarkLearningReverse?'':'selected'}" onclick="Glossary.setBookmarkLearningMode(false)">用語カード</button><button type="button" class="${bookmarkLearningReverse?'selected':''}" onclick="Glossary.setBookmarkLearningMode(true)">逆引き</button></div><div id="list-container" class="learning-bookmark-list">${bookmarkLearningListHtml()}</div>`;syncFloatingNav();requestAnimationFrame(()=>scrollTo(0,restoreScroll?bookmarkListScrollY:0))}
function setBookmarkLearningMode(isReverse){bookmarkLearningReverse=Boolean(isReverse);renderBookmark()}
function startLearningBookmarkSession(preferredId=0){const terms=bookmarkedTerms();if(!terms.length){renderBookmark();return}bookmarkListScrollY=window.scrollY;screen='session';flashcardMode=true;reverseMode=bookmarkLearningReverse;reverseRevealed=false;sessionModeKey='learningBookmarks';flashCategory='';session=[...terms];sessionIndex=Math.max(0,session.findIndex(term=>term.id===Number(preferredId)));sessionHistory=[sessionIndex];sessionHistoryIndex=0;sessionId=`lb-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;statusSessionMode=false;isTodaySession=false;todayQuizMode=false;todayAnswers=new Map();evaluatedIds=new Set();sessionStats={label:reverseMode?'ブックマーク逆引き':'ブックマーク用語カード',sourceStatus:'',studied:0,correct:0,firstCorrect:0,secondCorrect:0,wrong:0,unable:0,safe:0,caution:0,danger:0};prepareTerm();renderSession();scrollTo(0,0)}
...
function flashcardFields(term){return `${textField(sectionHeading('definition'),term.definition,'',true)}${listField(sectionHeading('exam'),term.exam,'exam-points')}${term.mixup?.length?mixupFieldHtml(term,'div',true):''}`}
...
function flashNext(){
  ...
  if(sessionHistoryIndex<sessionHistory.length-1){
    ...
    sessionIndex++;
    pushSessionHistory();
  }else if(sessionModeKey==='learningBookmarks'){
    flashSwipeLocked=false;
    renderBookmark(true);
    return;
  }else if(sessionModeKey==='statusLearning'){
    flashSwipeLocked=false;
    renderStatusList(activeStatusListKey,listTerms,activeStatusListLabel,true);
    return;
  }else if(sessionModeKey==='searchLearning'){
    flashSwipeLocked=false;
...
function flashPrevious(){
  if(!flashcardMode||flashSwipeLocked)return;
  if(sessionHistoryIndex===0&&sessionModeKey==='learningBookmarks'){renderBookmark(true);return}
  if(sessionHistoryIndex===0&&sessionModeKey==='statusLearning'){renderStatusList(activeStatusListKey,listTerms,activeStatusListLabel,true);return}
  if(sessionHistoryIndex===0&&sessionModeKey==='searchLearning'){restoreSearchResults();return}
...
function openSearchedTerm(id){if(!activeSearchTerms.length)return;activeSearchScrollY=window.scrollY;screen='session';flashcardMode=true;reverseMode=false;reverseRevealed=false;sessionModeKey='searchLearning';flashCategory='';session=[...activeSearchTerms];sessionIndex=Math.max(0,session.findIndex(term=>term.id===Number(id)));sessionHistory=[sessionIndex];sessionHistoryIndex=0;sessionId=`sr-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;statusSessionMode=false;isTodaySession=false;todayQuizMode=false;todayAnswers=new Map();evaluatedIds=new Set();sessionStats={label:'検索結果',sourceStatus:'',studied:0,correct:0,firstCorrect:0,secondCorrect:0,wrong:0,unable:0,safe:0,caution:0,danger:0};prepareTerm();renderSession();scrollTo(0,0)}
...
function exitCurrent(){if(isTodaySession){todayMeta().activeSession=null;saveTodayMeta()}if(sessionModeKey==='todayBookmarks')renderTodayBookmarks(true);else if(sessionModeKey==='learningBookmarks')renderBookmark(true);else if(sessionModeKey==='statusLearning')renderStatusList(activeStatusListKey,listTerms,activeStatusListLabel,true);else if(sessionModeKey==='searchLearning')restoreSearchResults();else if(sessionModeKey==='categoryAll')restoreHomePosition(categoryHomeScrollY);else renderHome()}
...
function renderLearningCard(){
 ...
 const details=`<section class="learning-details"><section><h2 class="definition">${sectionHeading('definition')}</h2><div${multilineAttr(defText)}>${defText}</div></section>${term.exam?.length?`<section><h2 class="point">${sectionHeading('exam')}</h2><div${multilineAttr(examText)}>${examText}</div></section>`:''}${term.mixup?.length?mixupFieldHtml(term,'h2',!reverse||reverseRevealed):''}</section>`;
...
function startLearningMode(isReverse,preferredId=0){
 ...
 screen='session';flashcardMode=true;reverseMode=isReverse;reverseRevealed=false;sessionModeKey='';
