...
const APP_VERSION='3.0.169',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function todayTerms(){const meta=todayMeta(),previous=new Set(meta.lastSession.ids),round=roundMeta().current,all=[...new Map(data.terms.filter(term=>q[term.id]&&round.correctAnswers[term.id]!==true).map(term=>[term.id,term])).values()],picked=[],pickedIds=new Set(),add=term=>{if(picked.length<10&&!pickedIds.has(term.id)){picked.push(term);pickedIds.add(term.id)}};const neverAnswered=shuffle(all.filter(term=>(termState(term)?.recentAnswers||[]).length===0)),needsReview=shuffle(all.filter(term=>states[round.answers[term.id]]&&!previous.has(term.id)&&['caution','danger'].includes(statusBucket(term)))),unanswered=shuffle(all.filter(term=>!states[round.answers[term.id]]&&!previous.has(term.id))),otherReview=shuffle(all.filter(term=>states[round.answers[term.id]]&&!previous.has(term.id)));neverAnswered.forEach(add);needsReview.forEach(add);unanswered.forEach(add);otherReview.forEach(add);shuffle(all.filter(term=>!pickedIds.has(term.id))).forEach(add);return diversifyToday(picked)}
