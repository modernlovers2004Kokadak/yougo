...
const APP_VERSION='3.0.141',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
const EXAM_FIELDS=Object.freeze([
 {name:'関係法規・制度および運営管理',categories:['消費者基本法']},
 {name:'衛生管理',categories:['感染症法','感染症予防','消毒法','公衆衛生','地域保健法','健康増進法']},
 {name:'保健',categories:['皮膚','毛髪']},
 {name:'香粧品化学',categories:['香粧品成分']},
 {name:'文化論および理容技術理論',categories:['カッティング']}
]);
...
function categoryStatsHtml(){const rows=categoryStats(),total=rows.reduce((sum,row)=>sum+row.total,0),rowMap=new Map(rows.map(row=>[row.category,row]));return `<section class="category-stats"><h2 class="category-stats-title">分野別成績：計${total}語</h2>${EXAM_FIELDS.map((field,index)=>{const fieldRows=field.categories.map(category=>rowMap.get(category)).filter(Boolean),terms=fieldRows.flatMap(row=>row.terms),breakdown=categoryBreakdown(terms);return `<details class="field-row"><summary><span class="field-row-heading"><span class="field-row-name">${esc(field.name)}</span>${categoryBarHtml(breakdown)}</span><span class="field-row-meta">${terms.length}語<span class="field-row-arrow" aria-hidden="true">›</span></span></summary><div class="field-row-content"><div class="field-learning-actions" aria-label="${esc(field.name)}の学習方法"><button type="button" onclick="Glossary.startFieldQuiz(${index})">［3択］</button><button type="button" onclick="Glossary.startFieldLearning(${index},false)">［用語］</button><button type="button" onclick="Glossary.startFieldLearning(${index},true)">［逆引］</button></div>${fieldRows.map(categoryRowHtml).join('')}</div></details>`}).join('')}</section>`}
