...
const APP_VERSION='3.0.161',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function roundMeta(){const raw=learning[ROUND_META_KEY];if(raw&&typeof raw==='object'&&!Array.isArray(raw)){raw.total=Math.max(0,Number(raw.total)||0);raw.history=Array.isArray(raw.history)?raw.history:[];return raw}return learning[ROUND_META_KEY]={total:0,history:[]}}
...
function countRoundProgress(response){if(response.roundCounted)return;response.roundCounted=true;const meta=roundMeta();meta.total++;if(meta.total%data.terms.length===0){const completedRound=Math.floor(meta.total/data.terms.length),breakdown=categoryBreakdown(data.terms);if(!meta.history.some(item=>Number(item.round)===completedRound))meta.history.push({round:completedRound,at:dateTime(),safe:breakdown.safe,caution:breakdown.caution,danger:breakdown.danger,unlearned:breakdown.unlearned,total:breakdown.total})}saveLearning()}
