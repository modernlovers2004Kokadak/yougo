...
function mixupComparisonHtml(term,linkable=true){
 ...
 return `<div class="mixup-comparison" data-multiline>${group.terms.map(item=>`<section class="mixup-comparison-term"><div class="mixup-term-name">${render(item.name)}</div><ul>${(item.points||[]).map(point=>`<li><span aria-hidden="true">・</span><span>${render(String(point).replace(/^・/u,''))}</span></li>`).join('')}</ul></section>`).join('')}<section class="mixup-memory"><div>【覚え方】</div>${(group.memory||[]).map(value=>`<p>${esc(value)}</p>`).join('')}</section></div>`;
}
