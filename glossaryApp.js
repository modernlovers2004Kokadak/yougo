...
const APP_VERSION='3.0.164',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function roundMeta(){const raw=learning[ROUND_META_KEY];if(raw&&typeof raw==='object'&&!Array.isArray(raw)){let changed=false;raw.history=Array.isArray(raw.history)?raw.history:[];if(raw.schema===2&&raw.history.length===0&&data.terms.length===664){raw.history.push({round:1,at:'',safe:449,caution:82,danger:26,unlearned:107,total:664,partial:true,recovered:true});changed=true}raw.history=raw.history.map(item=>({...item,partial:Boolean(item.partial)||Number(item.unlearned)>0}));raw.completed=raw.history.reduce((maximum,item)=>Math.max(maximum,Number(item.round)||0),0);raw.current=raw.current&&typeof raw.current==='object'&&!Array.isArray(raw.current)?raw.current:{answers:{}};raw.current.answers=raw.current.answers&&typeof raw.current.answers==='object'&&!Array.isArray(raw.current.answers)?raw.current.answers:{};if(raw.schema!==3){raw.schema=3;changed=true}if(changed)saveLearning();return raw}return learning[ROUND_META_KEY]={schema:3,completed:0,history:[],current:{answers:{}}}}
...
function recordGlobalRoundEvaluation(term,status){if(!term||!states[status])return;const meta=roundMeta(),answers=meta.current.answers;answers[term.id]=status;const breakdown=globalCurrentBreakdown();if(breakdown.unlearned===0){meta.completed++;meta.history.push({round:meta.completed,at:dateTime(),safe:breakdown.safe,caution:breakdown.caution,danger:breakdown.danger,unlearned:0,total:breakdown.total});meta.current={answers:{}}}saveLearning()}
