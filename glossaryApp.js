...
const APP_VERSION='3.0.165',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function roundMeta(){const raw=learning[ROUND_META_KEY];if(raw&&typeof raw==='object'&&!Array.isArray(raw)){let changed=false;raw.history=Array.isArray(raw.history)?raw.history:[];if(raw.schema!==4){const current=stats();if(current.total===data.terms.length&&current.unlearned===0){const corrected={round:1,at:'',safe:current.safe,caution:current.caution,danger:current.danger,unlearned:0,total:current.total};const firstIndex=raw.history.findIndex(item=>Number(item.round)===1);if(firstIndex<0)raw.history.unshift(corrected);else if(raw.history[firstIndex].partial||raw.history[firstIndex].recovered||Number(raw.history[firstIndex].unlearned)>0)raw.history[firstIndex]={...raw.history[firstIndex],...corrected,partial:false,recovered:false};changed=true}raw.schema=4;changed=true}raw.completed=raw.history.reduce((maximum,item)=>Math.max(maximum,Number(item.round)||0),0);raw.current=raw.current&&typeof raw.current==='object'&&!Array.isArray(raw.current)?raw.current:{answers:{}};raw.current.answers=raw.current.answers&&typeof raw.current.answers==='object'&&!Array.isArray(raw.current.answers)?raw.current.answers:{};if(changed)saveLearning();return raw}return learning[ROUND_META_KEY]={schema:4,completed:0,history:[],current:{answers:{}}}}
...
function renderRoundHistory(){setAppScreen('round-history',{returnScreen:APP_SCREEN.HOME});flashcardMode=false;const history=[...roundMeta().history].reverse();app.innerHTML=`<section class="view-head"><span></span><h1>過去の周回成績</h1><span></span></section>${history.length?`<section class="round-history-list">${history.map(item=>`<div class="round-history-row"><span>${lapLabel(item.round)}</span>${categoryBarHtml(item)}<div class="round-history-counts"><span>安心 ${Number(item.safe)||0}語</span><span>勘 ${Number(item.caution)||0}語</span><span>無理 ${Number(item.danger)||0}語</span><span>未学習 ${Number(item.unlearned)||0}語</span></div></div>`).join('')}</section>`:'<div class="empty">過去の周回履歴はありません</div>'}`;syncFloatingNav();scrollTo(0,0)}
